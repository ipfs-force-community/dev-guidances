...
1. 发起 conversation
   ...
   ```flow
   s=>start: 发现问题行
   e=>end: 提交审阅结果(Submit review)
   op_select_comment=>operation: 打开 Review changes 面板选择 Comment 项
   op_select_request_changes=>operation: 打开 Review changes 面板选择 Request changes 项
   op_start_comment=>operation: 发起 review comment
   cond_level=>condition: 是 p2 事项
   s->op_start_comment->cond_level
   cond_level(yes)->op_select_comment->e
   cond_level(no)->op_select_request_changes->e
   ```
...
2. 解决 conversation
   ...
   ```flow
   s=>start: 阅读评论内容
   op_add_comment=>operation: 追加评论
   cond_agreed=>condition: 达成一致
   op_submit_changes=>operation: 提交修改
   op_comment_changes=>operation: 评论已修改
   op_check_changes=>operation: 检查修改结果
   cond_changes_checked=>condition: 修改结果符合预期
   e=>end: Resolve conversation
   
   s->cond_agreed
   cond_agreed(no)->op_add_comment
   cond_agreed(yes)->op_submit_changes
   op_add_comment->cond_agreed
   op_submit_changes->op_comment_changes->op_check_changes->cond_changes_checked
   cond_changes_checked(no)->op_add_comment
   cond_changes_checked(yes)->e
